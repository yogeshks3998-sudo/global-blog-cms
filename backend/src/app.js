...
const isAllowedOrigin = (origin) => {
  ...
  try {
    ...
    return protocol === 'http:' && isLocalNetwork;
  } catch {
    return false;
  }
};

const isPublicBlogApiRequest = (req) =>
  req.path.startsWith('/api/blogs') || req.path.startsWith('/api/public/blogs');
...
app.use(helmet());
app.use((req, res, next) => cors(getCorsOptions(req))(req, res, next));
app.options('*', (req, res, next) => cors(getCorsOptions(req))(req, res, next));
